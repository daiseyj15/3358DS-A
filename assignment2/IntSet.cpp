...
IntSet IntSet::unionWith(const IntSet& otherIntSet) const
{
   IntSet unionW;
        unionW = *this;
   for(int z=0; z<otherIntSet.used;z++)

       {
            if(unionW.contains(otherIntSet.data[z])== false)
                {
...
IntSet IntSet::intersect(const IntSet& otherIntSet) const
{
   IntSet intersec;
        ...
        for(int x=0;x<used;x++)
        {
            if(otherIntSet.contains(data[x])== true)
            {
                intersec.add(data[x]);
            }
...
IntSet IntSet::subtract(const IntSet& otherIntSet) const
{
   IntSet subT;
     ...
     for(int x=0;x<otherIntSet.used;x++)
        {
            if(subT.contains(otherIntSet.data[x])== true)
            {
