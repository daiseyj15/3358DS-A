...
{
   // to be implemented as part of Assignment 8
   size_type loc0 = hash(sWord) % capacity;
   if ( data[loc0].word == sWord ) return true;
   if ( data[loc0].word.empty() ) return false;
   size_type i = 1,
          loci = (loc0 + i*i) % capacity;
   while (true)
   {
      if ( data[loci].word == sWord ) return true;
      if ( data[loci].word.empty() ) return false;
      ++i;
      loci = (loc0 + i*i) % capacity;
   }
...
HashTable::size_type HashTable::hash(const string& word) const
{
   ...
   size_type hVal = 5381;
   ///size_type c;
   size_type i = 0, size = word.size();
   while (i < size)
   ///{
   ///   c = word[i];
   ///   hVal = ((hVal << 5) + hVal) + c; // hVal*33 + c
   ///   ++i;
   ///}
      hVal = ((hVal << 5) + hVal) + word[i++]; // hVal*33 + c
   return hVal;
...
void HashTable::insert(const string& sWord)
{
   // to be implemented as part of Assignment 8
   size_type loc0 = hash(sWord) % capacity;
   if ( data[loc0].word.empty() )
      data[loc0].word = sWord;
   else
   {
      size_type i = 1,
                loci = (loc0 + i*i) % capacity;
      while (true)
      {
         if ( data[loci].word.empty() )
         {
            data[loci].word = sWord;
            break;
         }
         ++i;
         loci = (loc0 + i*i) % capacity;
